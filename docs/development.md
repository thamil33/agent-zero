...
2. Prepare the python environment to run ScrAI in. (⚠️ This step assumes you have some Python runtime installed.) By clicking the python version in lower right corner (3.13.1 in my example), you should get a list of available environments. You can click the `+ Create Virtual Environment` button. You might be prompted to select the environment manager if you have multiple installed. I have venv and Conda, I will select Conda here. I'm also prompted for desired python version, I will select 3.12, that is known to work well.
...
- ✅ The ability to develop, debug, and test ScrAI features seamlessly

You're now ready to contribute to ScrAI, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
